...
const PCI_USB_BASE_CLASS_CODE: u8 = 0x0c;
const PCI_USB_SUB_CLASS_CODE: u8 = 0x03;
const PCI_OHCI_USB_PRGIF: u8 = 0x10;     // OHCI USB1.1
const PCI_UHCI_USB_PRGIF: u8 = 0x0;      // UHCI USB1.1
const PCI_EHCI_USB_PRGIF: u8 = 0x20;     // EHCI USB2.0
const PCI_XHCI_USB_PRGIF: u8 = 0x30;     // xHCI USB3.0
...
impl Usb
{
    ...
    pub fn init(&mut self, mode: UsbMode)
    {
        ...
        {
            if device.get_base_class_code() == PCI_USB_BASE_CLASS_CODE &&
               device.get_sub_class_code() == PCI_USB_SUB_CLASS_CODE
            {
                let mut prog_if = 0;

                match self.mode
                {
                    UsbMode::Ohci => prog_if = PCI_OHCI_USB_PRGIF,
                    UsbMode::Uhci => prog_if = PCI_UHCI_USB_PRGIF,
                    UsbMode::Ehci => prog_if = PCI_EHCI_USB_PRGIF,
                    UsbMode::Xhci => prog_if = PCI_XHCI_USB_PRGIF,
                }
    ...
    fn switch_ehci_to_xhci_mode(&self)
    {
        ...
        for device in PCI.lock().get_devices()
        {
            if device.get_vendor_id() == PCI_VENDOR_ID_INTEL &&
               ...
               device.get_program_interface_class_code() == PCI_EHCI_USB_PRGIF
            {
